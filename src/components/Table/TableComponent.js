...
import { importContactos, importContactosSuperusuario } from "../../services/ContactosService";
...
function TableComponent({
  ...
  useEffect(() => {
    if (fileRef.current?.files[0] !== undefined) {
      ...
      if (is_superuser === '1') {
        importContactosSuperusuario(file, u_id_empresa, u_usuario,setloading);
      }
      else{
        importContactos(file, u_id_empresa, u_usuario,setloading)
      }
  ...
  return (
    <>
      <div className="main-card mt-3 ml-2 card ">
        <div className="card-header pt-2 pb-2 base-background-gradient text-white">
          {title}
          {showSearch && (
